...
class BaseTrainer:
    ...
    def __init__(
        ...
        if isinstance(amp_dtype, str):
            amp_dtype = {"bfloat16": torch.bfloat16, "float16": torch.float16}.get(amp_dtype, None)
        self.amp_dtype = amp_dtype
        self._autocast_device = "cuda" if str(self.device).startswith("cuda") else "cpu"
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(self.train_dataloader, desc="train", total=self.epoch_len)):
            ...
            if batch_idx % self.log_step == 0:
                ...
                self.logger.debug(
                    "Train Epoch: {} {} Loss: {:.6f}".format(epoch, self._progress(batch_idx), batch["g_loss"].item())
                )
                self.writer.add_scalar("learning rate", self.g_lr_scheduler.get_last_lr()[0])
    ...
    def _clip_grad_norm(self):
        ...
        if self.config["trainer"].get("max_grad_norm", None) is not None:
            clip_grad_norm_(self.generator.parameters(), self.config["trainer"]["max_grad_norm"])
            clip_grad_norm_(self.discriminator.parameters(), self.config["trainer"]["max_grad_norm"])
    ...
    def _autocast_context(self):
        if self.amp_dtype is None:
            return nullcontext()
        return torch.autocast(self._autocast_device, dtype=self.amp_dtype)
